...
<p align="center">
  <a href="https://github.com/duanlanqunshan/QMAI/releases">
    <img src="https://img.shields.io/github/v/release/duanlanqunshan/QMAI?style=flat-square" alt="Release" />
  </a>
...
前往 [GitHub Releases](https://github.com/duanlanqunshan/QMAI/releases) 下载最新版本安装包。
...
QMAI **不收集、不上传任何 API 密钥**，所有模型密钥只保存在运行本机的 Tauri 用户数据目录中，且默认以密文形式存储。设置页中的密钥输入框新增了「小眼睛」切换显示/隐藏，方便排查但不会被误带出本机。

仓库自身做了三层保护：

1. `.gitignore` 排除 `release-portable/*.exe`、`dist/`、`src-tauri/target/`、`.env*`、debug 抓包文件（`.network-request` / `.network-response`）、本地 `tools/` 与 `备份/`。
2. `scripts/check-no-secrets.sh` 在 commit 前对仓库文件做模式匹配扫描，命中 `sk-…` / `nvapi-…` / `Bearer …` 等便会失败退出。
3. 推荐在 GitHub 仓库设置里同时开启 **Secret scanning** + **Push protection**，作为最后一道防线。

如果你打算 fork / PR，也请避免把本地密钥、`*.exe`、构建产物放进提交。
