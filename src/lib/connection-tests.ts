...
export async function testEmbeddingConnection(cfg: EmbeddingConfig): Promise<ProviderTestResult> {
  ...
  const vector = await fetchEmbedding("LLM Wiki embedding connection test.", cfg, 0, "query")
  if (!vector) {
    return {
      ok: false,
...
export async function testEmbeddingFunction(cfg: EmbeddingConfig): Promise<ProviderTestResult> {
  const first = await fetchEmbedding("LLM Wiki functional embedding test: apple banana graph.", cfg, 0, "query")
  const second = await fetchEmbedding("LLM Wiki functional embedding test: apple banana graph.", cfg, 0, "query")
  if (!first || !second) {
    return {
      ok: false,
