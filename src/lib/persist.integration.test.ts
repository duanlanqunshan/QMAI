...
describe("review persistence — round-trip", () => {
  ...
  it("creates the .qmai directory on first save", async () => {
    expect(await fileExists(`${tmp.path}/.qmai`)).toBe(false)
    await saveReviewItems(tmp.path, [makeReview()])
    expect(await fileExists(`${tmp.path}/.qmai/review.json`)).toBe(true)
  })
  ...
  it("returns empty array when the file is corrupted JSON", async () => {
    await writeFileRaw(`${tmp.path}/.qmai/review.json`, "{not valid json")
    const loaded = await loadReviewItems(tmp.path)
    expect(loaded).toEqual([])
  })
...
describe("chat persistence — round-trip (new format)", () => {
  function makeConv(id: string, title: string = "conv"): Conversation {
    return { id, title, createdAt: 0, updatedAt: 1, deAiMode: false }
  }
  ...
  it("writes conversations.json + per-conversation chats/<id>.json", async () => {
    ...
    expect(await fileExists(`${tmp.path}/.qmai/conversations.json`)).toBe(true)
    expect(await fileExists(`${tmp.path}/.qmai/chats/c1.json`)).toBe(true)
    expect(await fileExists(`${tmp.path}/.qmai/chats/c2.json`)).toBe(true)
  })
  ...
  it("skips missing per-conversation files without throwing", async () => {
    ...
    await writeFileRaw(
      `${tmp.path}/.qmai/conversations.json`,
      JSON.stringify([makeConv("c1"), makeConv("c2")]),
    )
    await writeFileRaw(
      `${tmp.path}/.qmai/chats/c1.json`,
...
describe("chat persistence — .qmai legacy file-format fallback", () => {
  function makeConv(id: string): Conversation {
    return { id, title: "t", createdAt: 0, updatedAt: 1, deAiMode: false }
  }
  ...
  it("falls back to chat-history.json flat-array format", async () => {
    ...
    await writeFileRaw(
      `${tmp.path}/.qmai/chat-history.json`,
  ...
  it("new format wins over legacy when both exist", async () => {
    ...
    await writeFileRaw(`${tmp.path}/.qmai/chats/new.json`, "[]")
