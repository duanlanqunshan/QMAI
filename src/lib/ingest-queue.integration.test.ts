 ...
 *
 * Exercises the actual `.qmai/ingest-queue.json` write/read round-trip
...
async function readQueueFile(): Promise<string> {
  return readFileRaw(`${tmp.path}/.qmai/ingest-queue.json`)
}

describe("ingest-queue persistence — write", () => {
  it("writes .qmai/ingest-queue.json after enqueue", async () => {
  ...
  it("auto-creates .qmai/ directory when it doesn't exist", async () => {
    expect(await fileExists(`${tmp.path}/.qmai`)).toBe(false)
    await enqueueIngest(TEST_ID_A, "x.md")
    await waitFor(() => fileExists(`${tmp.path}/.qmai/ingest-queue.json`))
  })
...
describe("ingest-queue persistence — restore round-trip", () => {
  ...
  it("converts 'processing' back to 'pending' when restoring (interrupted app close)", async () => {
    ...
    await writeFileRaw(
      `${tmp.path}/.qmai/ingest-queue.json`,
