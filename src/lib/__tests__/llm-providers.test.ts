...
describe("Origin header — local LLM CORS workaround", () => {
  ...
  it("strips a pasted /embeddings tail from custom chat endpoints before appending /chat/completions", () => {
    const cfg = getProviderConfig({
      provider: "custom",
      apiKey: "",
      model: "qwen3",
      ollamaUrl: "",
      customEndpoint: "https://api.siliconflow.cn/v1/embeddings",
      maxContextSize: 8192,
      apiMode: "chat_completions",
    } as RealLlmConfig)

    expect(cfg.url).toBe("https://api.siliconflow.cn/v1/chat/completions")
  })
